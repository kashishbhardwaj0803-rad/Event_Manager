...
export default function AttendeeLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!profile || profile.role !== "attendee") {
      setError("This account is not an attendee account. Please use the correct portal.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={cardWrap}>
        ...
        <h1 style={heading}>Attendee Sign In</h1>
        <p style={subheading}>Discover and register for events.</p>
        ...
        <p style={{ marginTop: 24, textAlign: "center", fontSize: 13, color: "rgba(255,255,255,0.4)" }}>
          No account?{" "}
          <Link href="/attendee/register" style={{ color: "#4ADE80", fontWeight: 600, textDecoration: "none" }}>Register Free</Link>
        </p>
      </div>
    </div>
  );
