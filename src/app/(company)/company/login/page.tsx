...
import { signIn, getUser } from "@/lib/supabase";
...
export default function CompanyLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!profile || profile.role !== "company") {
      setError("This account is not a company account. Please use the correct portal.");
      setLoading(false);
      return;
    }
  ...
  return (
    <div style={pageStyle}>
      ...
      <div style={cardWrap}>
        ...
        <h1 style={heading}>Company Sign In</h1>
        <p style={subheading}>Manage your events and attendees.</p>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 28 }}>
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input type="email" value={form.email} onChange={set("email")} placeholder="jane@company.com" required style={inputStyle} />
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ position: "relative" }}>
              ...
              <input type={showPw ? "text" : "password"} value={form.password} onChange={set("password")} placeholder="Your password" required style={{ ...inputStyle, paddingRight: 44 }} />
        ...
        <p style={{ marginTop: 24, textAlign: "center", fontSize: 13, color: "rgba(255,255,255,0.4)" }}>
          No account yet?{" "}
          <Link href="/company/register" style={{ color: "#ADC6FF", fontWeight: 600, textDecoration: "none" }}>Register Company</Link>
        </p>
      </div>
    </div>
  );
...
const logoIcon: React.CSSProperties = {
  width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#ADC6FF,#4B8EFF)",
...
const errorBox: React.CSSProperties = {
  ...
  borderRadius: 10, padding: "10px 14px", fontSize: 13, color: "#FFB0B0", marginBottom: 8, marginTop: 16,
};
...
const submitBtn: React.CSSProperties = {
  ...
  background: "linear-gradient(135deg,#4B8EFF,#ADC6FF)", border: "none",
  color: "#0a0a10", fontSize: 14, fontWeight: 700, cursor: "pointer",
